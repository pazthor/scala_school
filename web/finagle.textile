...
"Finagle":https://github.com/twitter/finagle Es un sistema de  _llamado de Pocedmiento Remoto_ de Twitter(*RPC* por sus siglas en inglés). "Este":https://blog.twitter.com/2011/finagle-a-protocol-agnostic-rpc-system artículo de nuestro blog explica los motivos y los principios del diseño del nucleo, el  "finagle README":https://github.com/twitter/finagle/blob/master/README.md contiene una documentación más detallada. El objetivo de Finagle es desarrollar clientes y servidores robustos de una manera sencilla.

* "REPL":#repl
* "Futuro":#Future: "Composición secuencial":#futsequential, "Composición Concurrente":#futconcurrent, "Ejemplo de Composición: Límite de velocidad de memoria caché":#combined_combinator_example_cache, "Ejemplo de composición: Thumbnail Fetcher":#combined_combinator_thumbnail
* "Servicio":#Service
* "Ejemplo de un cliente":#client
* "Ejemplo de un servidor":#server
* "Filtros":#Filter
* "Constructores *(Builder)":#Builder
* "Sin bloqueos (Don't Block)":#DontBlock ( A menos que lo hagas de la manera correcta)

h2(#repl). Finagle REPL

Vamos a analizar algo de código que no es parte estandar de Scala, si te gusta usar la terminal para aprender, posiblemente te soprendas de como obtener el REPL de Scala para saber acerca 
